...
export default function AdminPage() {
  ...
  const handleExtend = async () => {
    if (selected.size === 0 || !token) return;
    const input = window.prompt(`${selected.size}명의 이용 기간을 며칠 연장하시겠습니까?`, "7");
    if (input === null) return;
    const days = Number(input.trim());
    if (!Number.isFinite(days) || days <= 0 || days > 3650) {
      alert("1~3650 사이의 숫자를 입력해 주세요.");
      return;
    }
    setWorking(true);
    setError(null);
    try {
      const res = await fetch(`/api/admin/extend`, {
        ...
        body: JSON.stringify({ ids: Array.from(selected), days }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || "연장 실패");
        return;
      }
      await fetchUsers(tab);
    } catch (e: any) {
            ...
            <div className="flex items-center gap-2">
              ...
              {tab === "approved" && (
                <button
                  ...
                  disabled={selected.size === 0 || working}
                  className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[11px] font-black disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm"
                >
                  {working ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CalendarPlus className="w-3.5 h-3.5" />}
                  기간 연장 ({selected.size})
                </button>
              )}
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                  ...
                  {tab === "approved" && <th className="px-4 py-3 text-left">만료일</th>}
                  {tab === "approved" && <th className="px-4 py-3 text-left">남은 일수</th>}
                      ...
                      {tab === "approved" && (
                        <td className="px-4 py-4 text-slate-500 font-bold tabular-nums text-xs">
                          {u.role === "admin"
                            ? "—"
                            : u.expires_at
                            ? new Date(u.expires_at).toLocaleString("ko-KR")
                            : "—"}
                        </td>
                      )}
                      {tab === "approved" && (
                        <td className="px-4 py-4">
                          <DaysLeftBadge expiresAt={u.expires_at} isAdmin={u.role === "admin"} />
                        </td>
                      )}
                      <td className="px-4 py-4">
...
function DaysLeftBadge({ expiresAt, isAdmin }: { expiresAt: string | null; isAdmin: boolean }) {
  if (isAdmin) {
    return <span className="px-2.5 py-1 bg-slate-100 text-slate-500 text-[10px] font-black rounded-md uppercase">무제한</span>;
  }
  if (!expiresAt) {
    return <span className="px-2.5 py-1 bg-slate-100 text-slate-500 text-[10px] font-black rounded-md uppercase">—</span>;
  }
  const diffMs = new Date(expiresAt).getTime() - Date.now();
  if (diffMs <= 0) {
    return <span className="px-2.5 py-1 bg-rose-50 text-rose-600 text-[10px] font-black rounded-md uppercase">만료</span>;
  }
  ...
  return <span className={`px-2.5 py-1 ${cls} text-[10px] font-black rounded-md tabular-nums`}>{days}일</span>;
}
